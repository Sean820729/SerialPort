using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Windows.Forms;
using System.IO;
using System.Threading.Tasks;
using System.Drawing;

namespace Yuan
{
    public static class Package
    {
        public static string[] GetCOMPorts()
        {
            string[] ComPortList = System.IO.Ports.SerialPort.GetPortNames();	//抓目前有連接裝置的COMPort
            string SwapBuf;
            for (int i = 0; i < ComPortList.Length; i++)
            {						//放進combobox並排序
                if (!char.IsDigit(ComPortList[i][ComPortList[i].Length - 1]))
                {	//如果最後一位不是數字就把最後一個字元刪掉，framework 4.0以上好像不會有問題
                    ComPortList[i] = ComPortList[i].Substring(0, ComPortList[i].Length - 1);
                }

                //排序，由小排到大
                for (int j = 0; j < i; j++)
                {
                    if (Convert.ToInt32(ComPortList[i].Substring(3)) < Convert.ToInt32(ComPortList[j].Substring(3)))
                    {
                        SwapBuf = ComPortList[j];
                        ComPortList[j] = ComPortList[i];
                        ComPortList[i] = SwapBuf;
                    }
                }
            }

            return ComPortList;
        }

        public static bool Check(byte[] ReCBuf, byte[] PackageHeader, int PackageLength, byte[] PackageTail)
        {
            bool isPackage = true;
            for (int i = 0; i < PackageHeader.Length; i++)
            {
                if (ReCBuf[i] != (byte)PackageHeader[i])
                {
                    isPackage = false;
                    break;
                }
            }

            if (isPackage)
            {
                for (int i = PackageLength - PackageTail.Length; i < PackageLength; i++)
                {
                    if (ReCBuf[i] != (byte)PackageTail[i - PackageLength + PackageTail.Length])
                    {
                        isPackage = false;
                        break;
                    }
                }
            }

            return isPackage;
        }

        public static bool Check(List<byte> ReCBuf, byte[] PackageHeader, int PackageLength, byte[] PackageTail)
        {
            bool isPackage = true;
            for (int i = 0; i < PackageHeader.Length; i++)
            {
                if (ReCBuf[i] != (byte)PackageHeader[i])
                {
                    isPackage = false;
                    break;
                }
            }

            if (isPackage)
            {
                for (int i = PackageLength - PackageTail.Length; i < PackageLength; i++)
                {
                    if (ReCBuf[i] != (byte)PackageTail[i - PackageLength + PackageTail.Length])
                    {
                        isPackage = false;
                        break;
                    }
                }
            }

            return isPackage;
        }



        public static void ShiftAndGetNew(ref byte[] raw, int index, ref System.Collections.Queue RecQueue)
        {
            if (index == 0)
            {
                for (int i = 0; i < raw.Length; i++)
                    raw[i] = Convert.ToByte(RecQueue.Dequeue());
            }
            else if (index > 0)
            {
                System.Array.Copy(raw, index, raw, 0, raw.Length - index);
                for (int i = index; i > 0; i--)
                    raw[raw.Length - i] = Convert.ToByte(RecQueue.Dequeue());
            }
            else
            {
                for (int i = 0; i < raw.Length; i++)
                    raw[i] = Convert.ToByte(RecQueue.Dequeue());
            }
        }
        public static int ShiftAndClear(ref byte[] Buf, int ProtocolLength)
        {
            if (ProtocolLength < Buf.Length)
                System.Array.Copy(Buf, ProtocolLength, Buf, 0, Buf.Length - ProtocolLength);
            System.Array.Clear(Buf, Buf.Length - ProtocolLength, ProtocolLength);
            return Buf.Length - ProtocolLength;
        }

    }

    public static class Serial
    {
        public static void SerialportReceiveQueue(System.IO.Ports.SerialPort Serialport, ref System.Collections.Queue RecQueue)
        {
            int BytesCont = Serialport.BytesToRead; //將全部收到的資料都塞到Queue，因為一直去讀取IO會很花時間
            byte[] RecBuf = new byte[BytesCont];
            try
            {
                Serialport.Read(RecBuf, 0, BytesCont);
                for (int i = 0; i < BytesCont; i++)
                    RecQueue.Enqueue(RecBuf[i]);
            }
            catch
            {

                throw;
            }


        }
        public static void CheckAndDisconnection(System.IO.Ports.SerialPort Spt)
        {
            if (Spt.IsOpen) Spt.Close();
        }
        public static void CheckAndConnection(System.IO.Ports.SerialPort Spt, string PortName, ref string SavePort)
        {
            try
            {

                if (!Spt.IsOpen)
                {
                    Spt.PortName = PortName;
                    Spt.Open();
                    SavePort = PortName;
                }
            }
            catch
            {

                throw;
            }

        }
        public static void CheckAndConnection(System.IO.Ports.SerialPort Spt, string PortName)
        {
            if (!Spt.IsOpen)
            {
                Spt.PortName = PortName;
                Spt.Open();
            }
        }
        public static void CheckAndClear(System.IO.Ports.SerialPort Spt)
        {
            if (Spt.IsOpen)
            {
                string ReadBuf = Spt.ReadExisting();
            }
        }
    }

    public static class Math
    {
        /// <summary>
        /// weight of Var = 1/(ESALPHA + 1)
        /// </summary>
        /// <param name="VarES"></param>
        /// <param name="Var"></param>
        /// <param name="ESALPHA"></param>
        public static void ExponentialSmooth(ref float VarES, float Var, uint ESALPHA)
        {
            VarES = Var / (ESALPHA + 1) + VarES * ESALPHA / (ESALPHA + 1);
        }

        public static void ExponentialSmooth(ref double VarES, float Var, uint ESALPHA)
        {
            VarES = Var / (ESALPHA + 1) + VarES * ESALPHA / (ESALPHA + 1);
        }
    }

    public static class Array
    {
        /// <summary>
        /// if find, return first index
        /// else     return -1   
        /// </summary>
        /// <param name="Source"></param>
        /// <param name="Target"></param>
        /// <returns></returns>
        public static int ByteArrayIndexofByteArray(byte[] Source, byte[] Target)
        {
            int[] Index = new int[Target.Length];
            int i, j;
            for (i = 0; i <= Source.Length - Target.Length; i++)
            {
                for (j = 0; j < Target.Length; j++)
                {
                    if (Source[i + j] != Target[j]) break;
                }
                if (j == Target.Length) break;
            }
            return (i > Source.Length - Target.Length) ? -1 : i;
        }

        public static int ArrayIndexofArray<T>(List<T> Source, T[] Target)
        {
            int[] Index = new int[Target.Length];
            int i, j;
            for (i = 0; i <= Source.Count - Target.Length; i++)
            {
                for (j = 0; j < Target.Length; j++)
                {
                    if (Comparer<T>.Default.Compare(Source[i + j], Target[j]) != 0) break;
                }
                if (j == Target.Length) break;
            }
            return (i > Source.Count - Target.Length) ? -1 : i;
        }

        public static int ArrayIndexofArray<T>(T[] Source, T[] Target)
        {
            int[] Index = new int[Target.Length];
            int i, j;
            for (i = 0; i <= Source.Length - Target.Length; i++)
            {
                for (j = 0; j < Target.Length; j++)
                {
                    if (Comparer<T>.Default.Compare(Source[i + j], Target[j]) != 0) break;
                }
                if (j == Target.Length) break;
            }
            return (i > Source.Length - Target.Length) ? -1 : i;
        }
        public static int GetIndexofObjectArray(object[] Array, object Target)
        {
            for (int i = 0; i < Array.GetLength(0); i++)
            {
                if (Array[i].Equals(Target)) return i;
            }

            return -1;
        }

        public static int ArrayGetMinIndex<T>(T[] Array)
        {
            return System.Array.IndexOf(Array, Array.Min());
        }

        public static int ArrayGetMaxIndex<T>(T[] Array)
        {
            return System.Array.IndexOf(Array, Array.Max());
        }
    }




    public static class File
    {
        #region For WriteFile Fuction
        static List<string> PathList = new List<string>();
        static List<string> WriteFileBuf = new List<string>();
        static List<byte> RecBufList = new List<byte>();
        static List<StreamWriter> WriterList = new List<StreamWriter>();
        static List<Task> TaskList = new List<Task>();
        #endregion

        /// <summary>
        /// 將資料放到Write Buffer並寫入硬碟，關閉前要調用CloseFile()
        /// </summary>
        /// <param name="path">檔案路徑，Ex: @"D:/Test.txt"</param>
        /// <param name="Text">要寫入的內容，內容會被附加在檔案的最後面</param>
        public static void WriteFile(string path, string Text)
        {
            int index = (PathList != null) ? PathList.IndexOf(path) : -1;
            if (index < 0)
            {
                PathList.Add(path);
                WriteFileBuf.Add(Text);
                WriterList.Add(new StreamWriter(path, true));
                string DirPath = System.IO.Path.GetDirectoryName(path);


                if (!System.IO.Directory.Exists(DirPath))
                    System.IO.Directory.CreateDirectory(DirPath);
                TaskList.Add(WriterList[WriteFileBuf.Count - 1].WriteAsync(WriteFileBuf[WriteFileBuf.Count - 1]));
                WriteFileBuf[WriteFileBuf.Count - 1] = "";
            }
            else
            {


                WriteFileBuf[index] += Text;
                if (TaskList[index].IsCompleted)
                {
                    TaskList[index] = WriterList[index].WriteAsync(WriteFileBuf[index]);
                    WriteFileBuf[index] = "";
                }
            }
        }
        /// <summary>
        /// 將Write Buffer的資料入硬碟
        /// </summary>
        public static void CloseFile()
        {
            for (int i = 0; i < PathList.Count; i++)
            {
                if (WriteFileBuf[i] != "")
                {
                    while (TaskList[i].IsCompleted) ;
                    TaskList[i].Wait();
                    WriterList[i].WriteAsync(WriteFileBuf[i]);
                    WriteFileBuf[i] = "";
                }
            }

        }

        /// <summary>
        /// 將資料立刻寫入硬碟
        /// </summary>
        /// <param name="path">檔案路徑，Ex: @"D:/Test.txt"</param>
        /// <param name="Text">要寫入的內容，內容會被附加在檔案的最後面</param>
        public static void WriteFileInstantly(string path, string Text)
        {
            string DirPath = path.Substring(0, path.LastIndexOf('/'));
            if (!System.IO.Directory.Exists(DirPath))
                System.IO.Directory.CreateDirectory(DirPath);
            using (System.IO.StreamWriter Sw = new System.IO.StreamWriter(path, true))
            {
                Sw.Write(Text);
            }

        }
    }

    public static class UI
    {
        public static void LblAutoFontSize_SingleLine(Label Lbl, int MaxWidth, int MaxFontSize)
        {
            Lbl.AutoSize = true;
            Lbl.Font = new Font(Lbl.Font.FontFamily, MaxFontSize);
            while (Lbl.Width > MaxWidth) Lbl.Font = new Font(Lbl.Font.FontFamily, System.Math.Max(Lbl.Font.Size - 1, 0.000000001f));
            Lbl.AutoSize = false;
        }

        public static void LblAutoFontSize_SingleLine(Label Lbl)
        {
            Lbl.SuspendLayout();
            int MaxWidth = Lbl.Width, MaxHeight = Lbl.Height;
            Lbl.AutoSize = true;
            Lbl.Margin = new System.Windows.Forms.Padding(0);
            Lbl.Font = new Font(Lbl.Font.FontFamily, 300, Lbl.Font.Style);
            Lbl.Font = new Font(Lbl.Font.FontFamily, 300 * 1.2f / ((Lbl.Width + 1f) / (MaxWidth + 1f)), Lbl.Font.Style);
            while (Lbl.Width > MaxWidth || Lbl.Height > MaxHeight)
                Lbl.Font = new Font(Lbl.Font.FontFamily, System.Math.Max(Lbl.Font.Size - 1, 0.000000001f), Lbl.Font.Style);
            Lbl.AutoSize = false;
            Lbl.Size = new Size(MaxWidth, MaxHeight);
            Lbl.ResumeLayout();
        }

        public static void LblAutoFontSize(Label Lbl, int MaxFontSize)
        {
            float FontSize = MaxFontSize;
            float Xcnt = Convert.ToSingle((Lbl.Width - 2 * FontSize) / 1.4f / FontSize),
                  YCnt = Convert.ToSingle(Lbl.Height / 1.4f / FontSize);
            float TotalCnt = Xcnt * YCnt;
            if (Lbl.Text.Length > 0)
            {
                do
                {
                    FontSize = (TotalCnt != 0) ? FontSize / Convert.ToSingle(System.Math.Sqrt(Lbl.Text.Length / TotalCnt)) : FontSize - 1;
                    Xcnt = (Lbl.Width - 2 * FontSize) / 1.4f / FontSize;
                    YCnt = Lbl.Height / 1.4f / FontSize;
                    TotalCnt = Xcnt * YCnt;
                } while (TotalCnt < Lbl.Text.Length);
                Lbl.Font = new Font(Lbl.Font.FontFamily, FontSize);
            }
        }

    }
}
